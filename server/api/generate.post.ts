...
const FestangestellteSchema = object({
  id: string(),
  lohn_monat: NumberSchema,
  stunden_monat: NumberSchema,
  name: string(),
  job: JobSchema,
});
const FreelancerSchema = object({
  id: string(),
  lohn_stunde: NumberSchema,
  stunden_monat: NumberSchema,
  name: string(),
  job: JobSchema,
});
...
export default defineEventHandler(async (event) => {
  const { festangestellte, freelancer, project, target_date } =
    await readValidatedBody(event, (e: unknown) =>
      parse(GeneratorRequestSchema, e)
    );
  return balance(project, festangestellte, freelancer, target_date);
});
